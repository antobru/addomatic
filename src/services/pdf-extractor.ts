...
export class PdfExtractorService {
  ...
  async extract(filePath: string, overrides: PdfExtractorOptions = {}): Promise<PdfExtractionResult> {
    ...
    if (needsOcr) {
      return this.runOcr(filePath, pdf.numPages, pages, lang);
    }
  ...
  private async runOcr(
    ...
    // Se è un file immagine passato direttamente, OCR diretto.
    if (['.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.webp'].includes(ext)) {
      const text = await this.ocrImage(filePath, lang);
    ...
    // Suggerimento: se tutte le pagine sono vuote è un PDF scansionato puro.
    const allEmpty = nativePages.every((p) => p.length < this.ocrThreshold);
    if (allEmpty) {
      throw new Error(
        `Il file "${filePath}" sembra un PDF scansionato (${numPages} pagine, nessun testo nativo). ` +
